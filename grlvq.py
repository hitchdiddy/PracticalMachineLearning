...
class Grlvq():
    ...
    def fit(self,x,y):
        ...
        if not hasattr(self,'x'):
            ...
            self.relevance = np.ones(shape=(feat_dim))/feat_dim
            self.learning_rate_relevance = 0.02
        else:
    ...
    def fit_sample(self,xi,yi):
        ...
        if (num_prototypes_per_class == 0 or self.placement_strategy(xi,yi)) \
            ...
            mu = (d1 - d2) / (d1 + d2)
            # sigm = (1/(1+exp(-mu)))
            derive = exp(mu * self.strech_factor) / ((exp(mu * self.strech_factor) + 1)**2)
            #print('mu: ' + str(mu) + ' derive: ' + str(derive))
            # GLVQ
            d2_norm = d2 / ((d1 + d2)**2)
            d1_norm = d1 / ((d1 + d2)**2)
            self.prototypes[w1i] = w1 + self.learning_rate * derive * d2_norm * (xi - w1)
            self.prototypes[w2i] = w2 - self.learning_rate * derive * d1_norm * (xi - w2)

            # relevance update step
            self.relevance -= self.learning_rate_relevance * derive * (d2_norm * (xi - w1)**2 - d1_norm * (xi - w2)**2)
            # relevance normalization step
            self.relevance[self.relevance<0] = 0
            self.relevance = self.relevance / np.linalg.norm(self.relevance)
    ...
    def predict_proba(self,x,full_matrix=False,return_winning_prototype_i=False):
        ...
        ds = cdist(x,self.prototypes,self.relevance_dist)
        relsims = []
    ...
    def predict(self,x):
        ...
        if not hasattr(self,'labels') or len(set(self.labels)) < 2:
            return [-1] * len(x)
        return np.array(self.labels[np.argmin(cdist(x,self.prototypes,self.relevance_dist), axis=1)],np.int)

    def relevance_dist(self,x,y):
        return np.sum(self.relevance * (x - y)**2)
...
if __name__ == '__main__':
    ...
    x = np.random.multivariate_normal((0,0), [[1,0],[0,500]], 500)
    x = np.vstack((x,np.random.multivariate_normal((6.0,-1), [[1,0],[0,500]], 500)))
    ...
    # print(y_test)
    print('score grlvq: '+str(b.score(x_test,y_test)))

    print('relevance vector: ',b.relevance)
    ...
    # print(b.predict_proba(x_test))
    # print(y_test)
    print('score glvq: '+str(b.score(x_test,y_test)))
